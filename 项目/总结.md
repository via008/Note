...
return (
  <div className='model-table'>
    ...
    >
      {tableData.map((row, rowIndex) => row.map(
        (column, columnIndex) => render({rowIndex, columnIndex, rowNum, columnNum})
      ))}
    </div>
    <div className='model-table-item-label'>{rowNum} x {columnNum}</div>
  </div>
  )
}
